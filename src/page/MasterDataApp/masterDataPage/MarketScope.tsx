...
function MarketScope() {
    const [MScope, setMScope] = useState([
        {
            id: 1,
            kode: "",
            name: "Lokal (Kota/Kabupaten)",
            description: "Lokal (Kota/Kabupaten)",
        },
        {
            id: 2,
            kode: "",
            name: "Regional (Provinsi)",
            description: "Regional (Provinsi)",
        },
        {
            id: 3,
            kode: "",
    ...
    const handleEdit = (market: any) => {
        ...
        setFormKode(market.kode); //int 10
        setFormName(market.name);
        setFormDescription(market.description);
    ...
    const handleDelete = (id: number) => {
        const filtered = MScope.filter((item) => item.id !== id);
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {MScope.filter((market) =>market.name.toLowerCase().includes(search.toLowerCase()))
                                .map((market) => (
                                <tr
                                    key={market.id}
                                    className="hover:bg-gray-100 transition-colors duration-300 ease-in-out"
                                >
                                    <td className="py-3 pl-5 font-semibold text-black">{market.id}</td>
                                    <td className="py-3 text-black">{market.kode}</td>
                                    <td className="py-3 text-black">{market.name}</td>
                                    <td className="py-3 text-black">{market.description}</td>

                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-4 pr-1">
                                            <button onClick={() => handleEdit(market)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                                            ...
                                            <button onClick={() => handleDelete(market.id)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
