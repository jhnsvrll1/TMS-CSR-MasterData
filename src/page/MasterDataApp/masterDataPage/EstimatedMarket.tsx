...
function EstimatedMarket() {
    const [EstMarket, setEstMarket] = useState([
        {
            id: 1,
            kode: "",
            name: "Market Leader",
    ...
    const handleEdit = (estimation: any) => {
        ...
        setFormKode(estimation.kode); //int 10
        setFormName(estimation.name);
        setFormDescription(estimation.description);
    ...
    const handleSave = () => {
        if (editingId) {
            ...
            const newEstimation = {
                id: EstMarket.length + 1,
                kode: formKode,
                name: formName,
                description: formDescription,
            };

            setEstMarket([...EstMarket, newEstimation]);
        }
    ...
    const handleDelete = (id: number) => {
        const filtered = EstMarket.filter((item) => item.id !== id);
        ...
        setEstMarket(reindexed);
    };
    return (
        <>
            <div className="bg-white mt-6 px-4 py-3 border rounded-2xl">
                <div className="flex items-center justify-between">
                    <p className="font-semibold text-xl">Estimated Market Position</p>
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {EstMarket.filter((estimation) => estimation.name.toLowerCase().includes(search.toLowerCase()))
                                .map((estimation) => (
                                <tr
                                    key={estimation.id}
                                    className="hover:bg-gray-100 transition-colors duration-300 ease-in-out"
                                >
                                    <td className="py-3 pl-5 font-semibold text-black">{estimation.id}</td>
                                    <td className="py-3 text-black">{estimation.kode}</td>
                                    <td className="py-3 text-black">{estimation.name}</td>
                                    <td className="py-3 text-black">{estimation.description}</td>

                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-4 pr-1">
                                            <button onClick={() => handleEdit(estimation)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                                            ...
                                            <button onClick={() => handleDelete(estimation.id)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                ...
                : "bg-black/0 opacity-0"}`}>
                    ...
                    `}>
                        <h2 className="text-xl font-semibold mb-4">
                            Add New Estimated Market Position
                        </h2>
