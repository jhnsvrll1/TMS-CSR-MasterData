...
function EstimatedMarket (){
    const [estimatedMarkets, setEstimatedMarkets] = useState<any[]>([]);
    ...
    const fetchEstimatedMarkets = async () => {
        try {
            const response = await fetch("http://localhost:3000/api/master-data/market_positions");
            const result = await response.json();
            if (result.success) {
                setEstimatedMarkets(result.data);
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`http://localhost:3000/api/master-data/market_positions/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("http://localhost:3000/api/master-data/market_positions", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        if (!window.confirm("Are you sure you want to delete this estimated market position?")) return;
        try {
            await fetch(`http://localhost:3000/api/master-data/market_positions/${id}`, { method: "DELETE" });
            fetchEstimatedMarkets();
        } catch (error) {
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {estimatedMarkets.filter((item) => item.name.toLowerCase().includes(search.toLowerCase())).map((item, index) => (
            ...
            {showModal && (
                <div className={`fixed inset-0 flex items-center z-50 transition-opacity duration-200 justify-center ${animateModal ? "bg-black/40 opacity-100" : "bg-black/0 opacity-0"}`}>
                    <div className={`bg-white rounded-2xl shadow-lg w-125 p-6 relative transform transition-all duration-200 ${animateModal ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}>
                        <h2 className="text-xl font-semibold mb-4">{editingId ? "Edit Estimated Market Position" : "Add New Estimated Market"}</h2>
