...
function CompanySize (){
    const [ComSize,setComSize] = useState([
        {
            id: 1,
            kode: "",
            name: "Mikro (1-4 karyawan)",
            description: "Mikro (1-4 karyawan)",
        },
        {
            id: 2,
            kode: "",
            name: "Kecil (5-19 karyawan)",
            description: "Kecil (5-19 karyawan)",
        },
        {
            id: 3,
            kode: "",
            name: "Menengah (20-99 karyawan)",
            description: "Menengah (20-99 karyawan)",
        },
        {
            id: 4,
            kode: "",
            name: "Besar (100+ karyawan)",
            description: "Besar (100+ karyawan)",
        },
    ])
    ...
    const handleEdit = (company: any) => {
        ...
        setFormKode(company.kode); //int 10
    ...
    const handleDelete = (id: number) => {
        const filtered = ComSize.filter((item) => item.id !== id);
        ...
        setComSize(reindexed);
    };
    return(
        <>
            <div className="bg-white mt-6 px-4 py-3 border rounded-2xl">
                <div className="flex items-center justify-between">
                    <p className="font-semibold text-xl">Company Size</p>
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {ComSize.filter((company) => company.name.toLowerCase().includes(search.toLowerCase()))
                                .map((company) => (
                                <tr
                                    key={company.id}
                                    className="hover:bg-gray-100 transition-colors duration-300 ease-in-out"
                                >
                                    <td className="py-3 pl-5 font-semibold text-black">{company.id}</td>
                                    <td className="py-3 text-black">{company.kode}</td>
                                    <td className="py-3 text-black">{company.name}</td>
                                    <td className="py-3 text-black">{company.description}</td>

                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-4 pr-1">
                                            <button onClick={() => handleEdit(company)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                                            ...
                                            <button onClick={() => handleDelete(company.id)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
