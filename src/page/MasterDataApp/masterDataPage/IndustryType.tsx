...
function IndustryType (){
    const [IndType, setIndType] = useState([
        {
            ...
            description: "Retail / Perdagangan",
        },
        {
            id: 2,
            kode: "",
    ...
    const handleEdit = (industry: any) => {
        ...
        setFormKode(industry.kode);
        setFormName(industry.name);
        setFormDescription(industry.description);
    ...
    const handleDelete = (id: number) => {
        const filtered = IndType.filter((item) => item.id !== id);
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {IndType.filter((industry) => industry.name.toLowerCase().includes(search.toLowerCase()))
                                .map((industry) => (
                                <tr
                                    key={industry.id}
                                    className="hover:bg-gray-100 transition-colors duration-300 ease-in-out"
                                >
                                    <td className="py-3 pl-5 font-semibold text-black">{industry.id}</td>
                                    <td className="py-3 text-black">{industry.kode}</td>
                                    <td className="py-3 text-black">{industry.name}</td>
                                    <td className="py-3 text-black">{industry.description}</td>

                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-4 pr-1">
                                            <button onClick={() => handleEdit(industry)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                                            ...
                                            <button onClick={() => handleDelete(industry.id)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
