...
function AnnualRevenue (){
    ...
    const fetchAnnualRevenues = async () => {
        try {
            const response = await fetch("http://localhost:3000/api/master-data/revenue_ranges");
            const result = await response.json();
            if (result.success) {
                setAnnualRevenues(result.data);
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await fetch(`http://localhost:3000/api/master-data/revenue_ranges/${editingId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                await fetch("http://localhost:3000/api/master-data/revenue_ranges", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`http://localhost:3000/api/master-data/revenue_ranges/${id}`, { method: "DELETE" });
            fetchAnnualRevenues();
        } catch (error) {
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {annualRevenues.filter((item) => item.name.toLowerCase().includes(search.toLowerCase())).map((item, index) => (
