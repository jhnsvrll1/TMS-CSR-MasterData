...
function FinancialStatus() {
    ...
    const handleEdit = (finance: any) => {
        ...
        setFormKode(finance.kode); //int 10
        setFormName(finance.name);
        setFormDescription(finance.description);
    ...
    const handleSave = () => {
        if (editingId) {
            ...
            const newFinance = {
                id: FinStatus.length + 1,
                kode: formKode,
                name: formName,
                description: formDescription,
            };

            setFinStatus([...FinStatus, newFinance]);
        }
    ...
    const handleDelete = (id: number) => {
        const filtered = FinStatus.filter((item) => item.id !== id);
        ...
        setFinStatus(reindexed);
    };
    return (
        <>
            <div className="bg-white mt-6 px-4 py-3 border rounded-2xl">
                <div className="flex items-center justify-between">
                    <p className="font-semibold text-xl">Financial Status</p>
                ...
                <div className="max-h-80 overflow-y-auto mt-4 mb-4">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y text-gray-400">
                            {FinStatus.filter((finance) => finance.name.toLowerCase().includes(search.toLowerCase()))
                                .map((finance) => (
                                <tr
                                    key={finance.id}
                                    className="hover:bg-gray-100 transition-colors duration-300 ease-in-out"
                                >
                                    <td className="py-3 pl-5 font-semibold text-black">{finance.id}</td>
                                    <td className="py-3 text-black">{finance.kode}</td>
                                    <td className="py-3 text-black">{finance.name}</td>
                                    <td className="py-3 text-black">{finance.description}</td>

                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-4 pr-1">
                                            <button onClick={() => handleEdit(finance)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
                                            ...
                                            <button onClick={() => handleDelete(finance.id)} className="p-1 hover:bg-gray-300 rounded-lg transition-colors duration-300 ease-in-out">
